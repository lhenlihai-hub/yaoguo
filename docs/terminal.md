...
首屏显示“欢迎使用腰果”和小型字符图形，不在启动界面中展示底层依赖介绍。界面中间持续展示对话、DeepSeek 推理流、思考耗时、工具工作流与成品；底部提供多行输入框，状态栏显示完整工作空间路径、模型、思考强度、授权模式、API Key 与 token/cache 摘要。用户消息只使用蓝色字体，不使用背景色；Agent 回复按 Markdown 流式渲染。
...
默认会话与工作空间绑定。再次从同一目录启动会继续原会话，并恢复最近的可见对话；完整记录仍保存在腰果运行目录中。`/resume` 可直接打开或删除历史会话；删除会清理该会话的记录和受管任务数据，已发布成品会先归档，用户工作空间中的文件不会删除。`/new` 和 `--new` 可创建独立会话。
