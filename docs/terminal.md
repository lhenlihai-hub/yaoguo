...
界面由四部分组成：顶部显示版本与工作空间，中间持续展示对话、工具活动和成品，底部提供多行输入框，最下方状态栏显示模型、API Key 状态与 token/cache 摘要。用户消息使用蓝色背景，Agent 回复按 Markdown 流式渲染。

输入 `/` 会打开可搜索的命令菜单；方向键选择，`Enter` 确认，`Esc` 返回。内置命令如下：

```text
/model   选择 Pro / Flash，并设置或更新 DeepSeek API Key
...
默认会话与工作空间绑定。再次从同一目录启动会继续原会话，并恢复最近的可见对话；完整记录仍保存在腰果运行目录中。`--new` 可显式创建独立会话。
...
交互终端即使尚未配置 API Key 也能正常打开。输入 `/model` 后通过界内菜单选择 Pro 或 Flash，并可在隐藏输入框中设置或更新 DeepSeek API Key；密钥不会回显，也不会保留在终端历史中，保存后进入本机私有配置文件。环境变量 `DEEPSEEK_API_KEY` 仍然有效，本机配置优先于环境变量。
